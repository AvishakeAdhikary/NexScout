...
def test_claude_code_reads_resume_and_cover(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_openai_assistant_full_flow(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    fake_client.beta.threads.messages.list.return_value = SimpleNamespace(data=[SimpleNamespace(content=[msg_content])])
...
def test_openai_assistant_run_status_non_completed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    from nexscout.agent_backends import openai_assistant
    from nexscout.core.profile import Profile
